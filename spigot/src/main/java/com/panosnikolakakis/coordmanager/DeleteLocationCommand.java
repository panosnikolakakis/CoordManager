...
public class DeleteLocationCommand implements CommandExecutor, TabCompleter {

    private final Map<String, String> savedLocations;
    private final File configFile;

    public DeleteLocationCommand(Map<String, String> savedLocations, File configFile) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage("Usage: /locationdelete <LocationName>");
            return true;
        }
        ...
        if (savedLocations.containsKey(locationName)) {
            ...
            player.sendMessage("Location '" + ChatColor.GOLD + locationName + ChatColor.RESET + "' deleted.");

            // Save the updated locations to the configuration file
            saveLocationsToConfig(locationName);
        } else {
    ...
    private void saveLocationsToConfig(String locationName) {
        YamlConfiguration config = YamlConfiguration.loadConfiguration(configFile);

        // Remove the location from the configuration file
